...
export default function EditListingPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [amenRes, propRes] = await Promise.all([
          fetch('/api/amenities'),
        ...
        if (!saved && propJson.data) {
          ...
          setFormData({
            ...
            district: p.district || '',
            images: p.images || [],
            amenity_ids: p.amenities?.map((a: any) => a.id) || [],
            units: p.units || []
          });
        }
      } catch (error) {
  ...
  const addUnit = () => {
    setFormData((prev: any) => ({
      ...prev,
      units: [...prev.units, { name: '', unit_type: 'full_room', price_per_month: 0, total_capacity: 1, gender_restriction: 'any', available_from: new Date().toISOString().split('T')[0] }]
    }));
  };
  ...
  const handleSubmit = async (status: string) => {
    ...
    try {
      ...
      // 2. Update Amenities
      await fetch(`/api/properties/${params.id}/amenities`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      for (const unit of formData.units) {
        ...
        if (unit.id) {
          ...
          // Create new unit
          await fetch(`/api/properties/${params.id}/units`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(mappedUnit)
          });
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-6 space-y-10">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">Modify your property details below.</p>
      </div>
            ...
            <CardContent className="space-y-6">
              {formData.units.map((unit: any, index: number) => (
                <div key={index} className="p-6 border rounded-xl space-y-4 relative bg-muted/30">
                  {/* For edit, we might want to disable deleting existing units if they have bookings */}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <Button variant={unit.unit_type === 'full_room' ? 'default' : 'outline'} size="sm" onClick={() => updateUnit(index, 'unit_type', 'full_room')} className="flex-1">Room</Button>
                        <Button variant={unit.unit_type === 'bed_space' ? 'default' : 'outline'} size="sm" onClick={() => updateUnit(index, 'unit_type', 'bed_space')} className="flex-1">Bedspace</Button>
                      </div>
                    </div>
                    <div className="space-y-2">
        ...
        {step === 5 && (
          <Card className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <CardHeader>
              ...
              <CardDescription>Confirm all changes are correct before saving.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
            ...
            <CardFooter className="flex justify-between">
              <Button variant="outline" onClick={() => setStep(4)} disabled={isSubmitting}><ChevronLeft className="w-4 h-4 mr-2" /> Back</Button>
              <div className="flex gap-3">
                <Button variant="secondary" onClick={() => handleSubmit('draft')} disabled={isSubmitting}>Save as Draft</Button>
                <Button onClick={() => handleSubmit('published')} disabled={isSubmitting}>Update Listing</Button>
              </div>
            </CardFooter>
          </Card>
        )}
